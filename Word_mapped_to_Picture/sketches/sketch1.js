//Pushing an array of HSB colours in the pixel array
//Exporting these colours as an Adobe Swatch Exchange (ASE) file.
...
function draw() {
  ...
  for (var y = 0; y < height; y++) {
    for (var x = 0; x < width; x++) {
      ...
      var c = color(pixels[index], pixels[index + 1], pixels[index + 1], pixels[index + 1]);
      colours.push(c);
    }
  }
...
function keyPressed() {
  ...
  if(key == 'x' || key == 'X') writeFile(
    [gd.ase.encode(colours)],
    gd.timestamp(), 'ase');
}
